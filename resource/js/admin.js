...
(function() {
  (async function() {
    ...
    var roleLabel = adminRole === 'admin' ? '\u8D85\u7EA7\u7BA1\u7406\u5458' : '\u526F\u7BA1\u7406\u5458';
  ...
  async function loadUsers(body) {
    ...
    try {
      ...
      var html = '<table class="admin-table"><thead><tr><th>ID</th><th>\u90AE\u7BB1</th><th>\u7528\u6237\u540D</th><th>\u89D2\u8272</th><th>\u7B49\u7EA7</th><th>\u7ED1\u5B9A\u9009\u624B</th><th>\u6CE8\u518C\u65F6\u95F4</th><th>\u64CD\u4F5C</th></tr></thead><tbody>';
  ...
  window.deleteUser = async function(id) {
    ...
    var resp = await fetch('/api/admin/users/' + id, { method: 'DELETE' });
    var data = await resp.json();
  ...
  window.promoteSubAdmin = async function(id) {
    var username = prompt('\u8BBE\u7F6E\u8BE5\u7528\u6237\u4E3A\u526F\u7BA1\u7406\u5458\uFF1F\u8F93\u5165\u7528\u6237\u540D\u786E\u8BA4\uFF1A');
    if (!username) return;
  ...
  window.setLevel = async function(id, level) {
    var label = level === 2 ? 'Lv.2\uFF08\u53EF\u53D1\u6587\uFF09' : 'Lv.1';
    if (!confirm('\u786E\u5B9A\u5C06\u8BE5\u7528\u6237\u63D0\u5347\u4E3A ' + label + '\uFF1F')) return;
    var resp = await fetch('/api/admin/users/' + id + '/level', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function loadArticles(body) {
    ...
    try {
      var resp = await fetch('/api/admin/articles');
      var data = await resp.json();
      if (!data.ok) { body.innerHTML = '<p>\u52A0\u8F7D\u5931\u8D25</p>'; return; }
      ...
      var html = '<table class="admin-table"><thead><tr><th>\u6807\u9898</th><th>\u4F5C\u8005</th><th>\u72B6\u6001</th><th>\u65F6\u95F4</th><th>\u64CD\u4F5C</th></tr></thead><tbody>';
      data.articles.forEach(function(a) {
        ...
        var actions = '<button class="admin-btn danger" onclick="deleteArticle(\'' + a.slug + '\')">\u5220\u9664</button>';
        if (a.status === 'pending') {
  ...
  window.approveArticle = async function(slug) {
    var resp = await fetch('/api/admin/articles/' + slug + '/status', {
  ...
  window.deleteArticle = async function(slug) {
    ...
    var resp = await fetch('/api/admin/articles/' + slug, { method: 'DELETE' });
    var data = await resp.json();
    showToast(data.ok ? '\u5DF2\u5220\u9664' : (data.error || '\u5220\u9664\u5931\u8D25'), data.ok ? 'ok' : 'err');
    loadTab('articles');
  };
  ...
  async function loadBindings(body) {
    ...
    try {
      ...
      var players = resp[1].players || [];

      var html = '<table class="admin-table"><thead><tr><th>\u7528\u6237</th><th>\u7ED1\u5B9A\u9009\u624B</th><th>\u64CD\u4F5C</th></tr></thead><tbody>';
      users.filter(function(u) { return u.role !== 'admin'; }).forEach(function(u) {
        var bound = u.bound_players || [];
        html += '<tr>'
  ...
  window.showBindDialog = async function(userId, username) {
    ...
    var boundSlugs = (user && user.bound_players) ? user.bound_players.slice() : [];

    var checks = '';
    players.forEach(function(p) {
      var checked = boundSlugs.indexOf(p.slug) !== -1 ? 'checked' : '';
  ...
  window.saveBindings = async function() {
    var checks = document.querySelectorAll('#bindCheckboxes input[type="checkbox"]');
    var selected = [];
    checks.forEach(function(cb) { if (cb.checked) selected.push(cb.value); });
    var resp = await fetch('/api/admin/users/' + bindDialogUser, {
      method: 'PUT', headers: { 'Content-Type': 'application/json' },
  ...
  async function loadPlayers(body) {
    ...
    try {
      var resp = await fetch('/api/admin/players');
      var data = await resp.json();
      if (!data.ok) { body.innerHTML = '<p>\u52A0\u8F7D\u5931\u8D25</p>'; return; }
      var players = data.players || [];
      if (players.length === 0) { body.innerHTML = '<div class="admin-placeholder"><p>\u6682\u65E0\u9009\u624B</p></div>'; return; }

      var html = '<table class="admin-table"><thead><tr><th>ID</th><th>\u5934\u50CF</th><th>\u59D3\u540D</th><th>\u89D2\u8272</th><th>\u5934\u50CF URL</th><th>\u64CD\u4F5C</th></tr></thead><tbody>';
      players.forEach(function(p) {
        var avatarDisplay = p.avatar
  ...
  window.savePlayerAvatar = async function(slug) {
    var input = document.getElementById('av-' + slug);
    var avatar = input ? input.value.trim() : '';
    var resp = await fetch('/api/admin/players/' + slug, {
      method: 'PUT', headers: { 'Content-Type': 'application/json' },
  ...
  async function loadMatches(body) {
    ...
    try {
      var resp = await fetch('/api/admin/matches');
      var data = await resp.json();
      if (!data.ok) { body.innerHTML = '<p>\u52A0\u8F7D\u5931\u8D25</p>'; return; }
      var html = '<div style="margin-bottom:16px;"><button class="admin-btn primary" onclick="showMatchForm(null)">+ \u65B0\u589E\u8D5B\u4E8B</button></div>'
        + '<table class="admin-table"><thead><tr><th>\u8D5B\u4E8B</th><th>\u5BF9\u624B</th><th>\u65E5\u671F</th><th>\u7ED3\u679C</th><th>\u6BD4\u5206</th><th>\u9996\u9875\u5C55\u793A</th><th>\u64CD\u4F5C</th></tr></thead><tbody>';
      (data.matches || []).forEach(function(m) {
        var resultLabel = m.result === 'win' ? '\u80DC' : m.result === 'lose' ? '\u8D1F' : m.result === 'draw' ? '\u5E73' : '\u2014';
        html += '<tr>'
  ...
  window.showMatchForm = function(id) {
    editingMatchId = id;
    var defaults = { title: '', opponent: '', match_date: '', result: '', score: '', description: '', featured: 0 };

    if (id) {
      (async function() {
        var resp = await fetch('/api/admin/matches');
        var data = await resp.json();
        var m = (data.matches || []).find(function(x) { return x.id === id; });
        if (m) defaults = m;
        renderMatchForm();
      })();
    } else {
  ...
  window.saveMatch = async function() {
    ...
    var url = editingMatchId ? '/api/admin/matches/' + editingMatchId : '/api/admin/matches';
  ...
  window.deleteMatch = async function(id) {
    ...
    var resp = await fetch('/api/admin/matches/' + id, { method: 'DELETE' });
    var data = await resp.json();
    showToast(data.ok ? '\u5DF2\u5220\u9664' : (data.error || '\u5220\u9664\u5931\u8D25'), data.ok ? 'ok' : 'err');
    loadTab('matches');
  };
  ...
  async function loadHome(body) {
    body.innerHTML = '<div class="admin-loading">\u52A0\u8F7D\u2026</div>';
    try {
      var resp = await fetch('/api/admin/home');
      var data = await resp.json();
      if (!data.ok) { body.innerHTML = '<p>\u52A0\u8F7D\u5931\u8D25</p>'; return; }
      var secs = data.sections || [];
      var html = '<h3 style="margin:0 0 16px;font-size:18px;color:var(--text);">\u9996\u9875\u533A\u5757\u7F16\u8F91</h3>'
        + '<div style="display:grid;gap:14px;max-width:600px;">';
      secs.forEach(function(s) {
        var label = s.section_key === 'hero_title' ? '\u4E3B\u6807\u9898' : s.section_key === 'hero_subtitle' ? '\u526F\u6807\u9898' : s.section_key === 'about_text' ? '\u5173\u4E8E\u6211\u4EEC' : s.section_key;
